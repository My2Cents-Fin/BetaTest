-- ============================================
-- COMPLETE SCHEMA MIGRATION FOR DEV DATABASE
-- Run this entire file in My2Cents-dev SQL Editor
-- ============================================
...
CREATE TABLE IF NOT EXISTS public.household_members (
  ...
  UNIQUE(user_id) -- One user can only belong to one household
);
...
CREATE TABLE IF NOT EXISTS public.monthly_plans (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  household_id UUID REFERENCES public.households(id) ON DELETE CASCADE,
  plan_month DATE NOT NULL, -- First day of month (e.g., '2025-02-01')
...
CREATE TABLE IF NOT EXISTS public.budget_allocations (
  ...
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now(),
  UNIQUE(household_id, sub_category_id, plan_month)
);
...
CREATE TABLE IF NOT EXISTS public.transactions (
  ...
  transfer_to UUID REFERENCES public.users(id), -- Only for fund transfers
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
-- Next: Run the seed data script to populate categories
-- ============================================
